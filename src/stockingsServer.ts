...
export class StockingsServer implements SubscriptionTracker {
  ...
  private _acceptRequest: (req: StockingsConnectionRequest) => boolean
  ...
  constructor (options: StockingsServerOptions) {
    ...
    const algorithm = valueOrDefault(options.algorithm, 'HS256')
    ...
    const decryptionKey = isAlgorithmAsymmetric(algorithm) ? options.publicKey : privateKey
    ...
    if (typeof options.requestFilter === 'function') {
      ...
      this._acceptRequest = (req) => true
    }

    const disposeMetaAfter = valueOrDefault(options.disposeMetaAfter, HALF_HOUR)
    ...
    this._socketServer.on('request', (req) => {
      ...
      const connection = new StockingsConnection(req.accept(STOCKINGS_PROTOCOL), this)
      applyProtocol({
      ...
      const disposeMetaSubscription = connection.closeObservable.delay(disposeMetaAfter).subscribe(() => {
  ...
  getConnection (token: string, address: string): Promise<StockingsConnection> {
    return this._decoder(token).then((payload) => {
      const connection = this._connections.get(payload.clientId)
  ...
  sendData<T> (type: string, payload: T, cb?: (err?: any) => void): void {
    ...
    setImmediate(() => {
      const subscribers = this._connectionsBySubscription.get(type)
  ...
  registerSubscription (connection: StockingsConnection, type: string) {
    let connections: Map<string, StockingsConnection>
    if (this._connectionsBySubscription.has(type)) {
  ...
  unregisterSubscription (connection: StockingsConnection, type: string) {
    ...
    const connections = this._connectionsBySubscription.get(type)
